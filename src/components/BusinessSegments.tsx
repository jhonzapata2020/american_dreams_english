...
export const BusinessSegments: React.FC<BusinessSegmentsProps> = ({
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'HeartHandshake': return <HeartHandshake className="w-7 h-7 text-red-600" />;
    ...
    <section id="segmentos" className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-50 border-b border-slate-200 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          <span className="text-xs font-bold text-navy-900 uppercase tracking-widest bg-white px-3.5 py-1 rounded-full border border-slate-200 shadow-sm">
          ...
          {BUSINESS_SEGMENTS.map((seg) => (
            ...
            >
              <div>
                ...
                <p className="text-xs font-extrabold text-red-600 uppercase tracking-wider mb-4">
                  {seg.subtitle}
                </p>
              ...
              <div className="pt-4 border-t border-slate-100">
                <button
                  onClick={() => onSelectAction(seg.ctaAction)}
